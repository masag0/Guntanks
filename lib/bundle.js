...
var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); // const Game = require("./game");
...
var _tank = __webpack_require__(6);
...
var Game = function () {
  function Game() {
    ...
    this.tank = new _tank2.default('Player 1', 100, 0, ctx, ctxTerrain);
    this.tank2 = new _tank2.default('Player 2', 400, 0, ctx, ctxTerrain);
    this.tank3 = new _tank2.default('Player 3', 750, 0, ctx, ctxTerrain);
    this.tank4 = new _tank2.default('Player 4', 1050, 0, ctx, ctxTerrain);
  ...
  _createClass(Game, [{
    ...
    value: function render() {
      ...
      for (var _i = 0; _i < this.objects.length; _i++) {
        ...
        if (this.objects[_i] && (Math.abs(this.objects[_i].x) > 2000 || Math.abs(this.objects[_i].y) > 2000)) {
    ...
    value: function startTurns() {
      ...
      window.addEventListener('keypress', function (e) {
        ...
        if (e.key == '3') _this.currentTank.switchShotss(); //switch shot event listener
      });
      ...
      if (this.turnCounter % window.tanks.length === 0) {
        ...
        this.roundCounter++;
        this.turnCounter = 1;
        window.tanks = window.tanks.sort(function (a, b) {
      ...
      this.interval = setInterval(function () {
        if (_this.currentTank.turnOver) {
          ...
          _this.time--;
          document.getElementById('main-timer').innerHTML = _this.time;
        }
      }, 1000);
...
var Stage = function () {
  function Stage(ctxTerrain, canTerrain) {
    ...
    this.img.onload = function () {
      _this.ctx.drawImage(_this.img, 50, 400);
...
var UI = function () {
  ...
  _createClass(UI, [{
    ...
    value: function stopAdjustPower(tank) {
      ...
      // window.removeEventListener('keyup', this.game.bindKeyUp); //fixes multiple shots but causes maxpower firing bug
      if (!this.disabled) {
...
var _shell6 = __webpack_require__(7);
...
var Tank = function () {
  ...
  _createClass(Tank, [{
    ...
    value: function fire(power) {
      console.log('cooldown', this.ssCooldown);
      this.turnOver = true;
      ...
      if (this.face === 'right') {
        if (this.shellSelection === 1) {
          var shell = new _shell7.default(this.x + this.width, this.y - 5, power * this.shellSpeed * Math.cos(this.angle * Math.PI / 180), -power * this.shellSpeed * Math.sin(this.angle * Math.PI / 180), this.ctx, this.ctxTerrain);
          return shell;
        } else if (this.shellSelection === 2) {
          var _shell = new _shell9.default(this.x + this.width, this.y - 5, power * this.shellSpeed * Math.cos(this.angle * Math.PI / 180), -power * this.shellSpeed * Math.sin(this.angle * Math.PI / 180), this.ctx, this.ctxTerrain);
          return _shell;
        } else {
          this.shellSelection = 1; //reset selector after ss shot
          var _shell2 = new _shellss2.default(this.x + this.width + 2, this.y - 5, power * this.shellSpeed * Math.cos(this.angle * Math.PI / 180), -power * this.shellSpeed * Math.sin(this.angle * Math.PI / 180), this.ctx, this.ctxTerrain);
          return _shell2;
        }
      } else if (this.face === 'left') {
        if (this.shellSelection === 1) {
          var _shell3 = new _shell7.default(this.x - 16, this.y - 10, -power * this.shellSpeed * Math.cos(this.angle * Math.PI / 180), -power * this.shellSpeed * Math.sin(this.angle * Math.PI / 180), this.ctx, this.ctxTerrain);
          return _shell3;
        } else if (this.shellSelection === 2) {
          var _shell4 = new _shell9.default(this.x - 16, this.y - 10, -power * this.shellSpeed * Math.cos(this.angle * Math.PI / 180), -power * this.shellSpeed * Math.sin(this.angle * Math.PI / 180), this.ctx, this.ctxTerrain);
          return _shell4;
        } else {
          this.shellSelection = 1; //reset selector after ss shot
          var _shell5 = new _shellss2.default(this.x - 20, this.y - 10, -power * this.shellSpeed * Math.cos(this.angle * Math.PI / 180), -power * this.shellSpeed * Math.sin(this.angle * Math.PI / 180), this.ctx, this.ctxTerrain);
          return _shell5;
        }
      }
    }
...
var Shell = function () {
  function Shell(x, y, xVel, yVel, ctx, ctxTerrain) {
    ...
    this.radius = 8;
    this.ctx = ctx;
    this.weight = 0.055; //0.15 is about right
    // this.tanks = tanks; //array of all tanks
    this.damage = 100;
    this.hitObject = null;
    this.explosionRadius = 48; //for shot type 1
    this.ctxTerrain = ctxTerrain;
  ...
  _createClass(Shell, [{
    ...
    value: function render(ctx) {
      ...
      if (this.hitTest(this.x, this.y, this.radius * 2, this.radius * 2) === 'tank') {
        ...
        // console.log('cos',Math.cos(this.windAngle*Math.PI/180));
        // console.log('sin',Math.sin(this.windAngle*Math.PI/180));
...
var Shell2 = function (_Shell) {
  ...
  function Shell2(x, y, xVel, yVel, ctx, ctxTerrain) {
    ...
    _this.radius = 6;
    _this.ctx = ctx;
    _this.weight = 0.08; //0.15 is about right
    // this.tanks = tanks; //array of all tanks
    _this.damage = 200;
    _this.hitObject = null;
    _this.explosionRadius = 35; //for shot type 1
    _this.color = 'red';
...
var ShellSS = function (_Shell) {
  ...
  function ShellSS(x, y, xVel, yVel, ctx, ctxTerrain) {
    ...
    _this.radius = 10;
    _this.ctx = ctx;
    _this.weight = 0.07; //0.15 is about right
    // this.tanks = tanks; //array of all tanks
    _this.damage = 300;
    _this.hitObject = null;
    _this.explosionRadius = 70; //for shot type 1

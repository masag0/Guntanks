...
class Tank {
  ...
  hitTest (x, y, width, height) { //return true if collision
    ...
    for(let i = 0; i < array.length; i++) {
      ...
      if (
        this.ctx.getImageData(array[i][0],array[i][1],1,1).data[0] != 0 ||
        this.ctx.getImageData(array[i][0],array[i][1],1,1).data[1] != 0 ||
        this.ctx.getImageData(array[i][0],array[i][1],1,1).data[2] != 0 ||
        this.ctx.getImageData(array[i][0],array[i][1],1,1).data[3] != 0
      ) {
        return true;
      }
    }
  ...
  fire (power) {
    console.log(power);
    if (this.face === 'right') {
      const shell = new Shell(this.x+this.width, this.y, power*this.shellSpeed*Math.cos(this.angle*Math.PI/180), -power*this.shellSpeed*Math.sin(this.angle*Math.PI/180), this.shellRadius, this.ctx);
      return shell;
    } else if (this.face === 'left') {
      const shell = new Shell(this.x, this.y, -power*this.shellSpeed*Math.cos(this.angle*Math.PI/180), -power*this.shellSpeed*Math.sin(this.angle*Math.PI/180), this.shellRadius, this.ctx);
      return shell;
    }
  }
